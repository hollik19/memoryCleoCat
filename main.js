...
class Main {
    ...
    function window.addEventListener('error', (event) => {
                console.error('Global error:', event.error);
                this.handleError(event.error);
            });
    
    function window.addEventListener('unhandledrejection', (event) => {
                console.error('Unhandled promise rejection:', event.reason);
                this.handleError(event.reason);
            });
    
            // Mobile-specific error handling
    function if (Utils.isMobile()) {
    ...
    function components.forEach(({ name, instance }) => {
    function if (!instance) {
    ...
    function window.addEventListener('resize', Utils.debounce(() => {
                this.handleResize();
            }, 250));
    ...
    function if ('connection' in navigator) {
    function navigator.connection.addEventListener('change', () => {
                    this.handleConnectionChange();
                });
    }
    
            // Prevent context menu on long press for mobile
    function if (Utils.isTouchDevice()) {
    function document.addEventListener('contextmenu', (e) => {
    function if (e.target.closest('.game-container')) {
                        e.preventDefault();
    }
                });
    }
    ...
    function if (deviceInfo.memory < 4 || deviceInfo.cores < 4) {
                this.targetFPS = 30;
                this.frameInterval = 1000 / this.targetFPS;
    ...
    function if (deviceInfo.screen.isSmall) {
    ...
    function update(deltaTime) {
            // Update game systems
    function if (game && game.currentScreen === 'game') {
                // Game is running, all updates handled by individual systems
    }
    
    function if (minigame && minigame.isActive) {
                // Minigame updates are handled in its own loop
    }
    
            // Update audio system
    function if (audioManager && audioManager.isInitialized) {
    ...
    function if (currentTime - this.performanceMonitor.lastCheck >= 1000) {
    ...
    function if (fps < 20 && graphics && graphics.performanceMode !== 'low') {
                graphics.setPerformanceMode('low');
    ...
    function if (fps > 50 && graphics && graphics.performanceMode === 'low') {
    ...
    function if (fpsDisplay) {
                fpsDisplay.textContent = `FPS: ${this.performanceMonitor.currentFPS}`;
                
                // Color code FPS
    function if (this.performanceMonitor.currentFPS >= 50) {
                    fpsDisplay.style.color = '#00b894';
    function } else if (this.performanceMonitor.currentFPS >= 30) {
    ...
    function if (Utils.isMobile() && 'getBattery' in navigator) {
    ...
    function if (connection && connection.effectiveType) {
                // Adjust features based on connection speed
    function if (connection.effectiveType === 'slow-2g' || connection.effectiveType === '2g') {
    ...
    function navigator.getBattery().then((battery) => {
                // Reduce performance when battery is low
    function if (battery.level < 0.2 && !battery.charging) {
    function if (graphics && graphics.performanceMode !== 'low') {
                        graphics.setPerformanceMode('low');
    ...
    function if (!this.lastAutoSave || Date.now() - this.lastAutoSave > 30000) {
    function if (game) {
    ...
    function } else if (screenSize.isMedium) {
    ...
    function notifications.forEach(notification => {
    ...
    function if (game && game.gameBoard) {
    ...
    function if (audioManager && game) {
    function switch (game.currentScreen) {
                    case 'title':
    ...
    function if (minigame && game && game.currentScreen === 'minigame') {
                minigame.resume();
    }
    ...
    function pauseIndicator.addEventListener('click', () => {
                    this.resume();
                });
    ...
    function setTimeout(() => {
    function if (overlay.parentNode) {
                    overlay.remove();
    }
            }, 10000);
    }
    ...
    function Utils.showErrorDialog('Recovery failed. Please refresh the page.', () => {
                    location.reload();
                });
    }
    }
    ...
    function if (state && state.gameState) {
    ...
    function document.addEventListener('keydown', (e) => {
        // Ctrl+Shift+D for debug mode
    function if (e.ctrlKey && e.shiftKey && e.code === 'KeyD') {
            e.preventDefault();
            main.toggleDebugMode();
    }
        
        // Ctrl+Shift+R for recovery
    function if (e.ctrlKey && e.shiftKey && e.code === 'KeyR') {
            e.preventDefault();
    ...
    function if (main.initialized) {
            main.resume();
    }
    });
    
    function window.addEventListener('blur', () => {
    function if (main.initialized) {
            main.pause();
    }
    });
